...
export function NewHabitForm() {
    ...
    function handleToggleWeekDay(weekDay: number) {
        if(weekDays.includes(weekDay)) {
            ...
            const weekDaysWithAddedOnde = [... weekDays, weekDay]
            setWeekDays(weekDaysWithAddedOnde)
        }
    }
            ...
            <div className="flex flex-col gap-2 mt-3">
                {availableWeekDays.map((weekDay, index) => {
                    return (
                        <Checkbox.Root 
                            key={weekDay} 
                            className="flex items-center gap-3 group pl-0 pr-0 pb-0 pt-0 focus:outline-none"
